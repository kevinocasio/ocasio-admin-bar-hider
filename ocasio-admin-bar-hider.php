<?php
/**
 * Plugin Name: Ocasio Admin Bar Hider
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-admin-bar-hider/
 * Description: Hides the WordPress admin bar for non-administrators on the front end and keeps only essential links in the dashboard.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-admin-bar-hider
 */
...
define('OCASIO_ABH_VERSION', '1.0.0');
define('OCASIO_ABH_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_ABH_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_abh_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_abh_activate() {
	if (get_option('ocasio_abh_enabled') === false) {
		$legacy = get_option('ko_abh_enabled', 1);
		update_option('ocasio_abh_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_abh_activate');

/**
 * 2. Core Logic: Hide Front-End Admin Bar for Non-Admins
 */
function ocasio_abh_filter_admin_bar($show) {
	$enabled = get_option('ocasio_abh_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_abh_enabled', 1);
	}
	if ((int) $enabled === 1 && !current_user_can('administrator')) {
		// Hide completely on the frontend, but allow minimalist toolbar in admin area
		return is_admin();
	}
	return $show;
}
add_filter('show_admin_bar', 'ocasio_abh_filter_admin_bar');

/**
 * 3. Minimalist Backend Admin Bar for Non-Admins
 */
function ocasio_abh_minimalist_admin_bar($wp_admin_bar) {
	$enabled = get_option('ocasio_abh_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_abh_enabled', 1);
	}
	// Early bail if disabled or if user is an administrator
	if ((int) $enabled !== 1 || current_user_can('administrator')) {
		return;
	}

	// Essential nodes to keep: Site Name / Visit Site, Top Secondary container, My Account / Logout
	$keep = array('site-name', 'top-secondary', 'my-account');
	...
	foreach ($all_nodes as $node_id => $node) {
		// Strip top-level nodes that are not in the essential keep list
		if (empty($node->parent) && !in_array($node_id, $keep, true)) {
			$wp_admin_bar->remove_node($node_id);
		}
	}
}
add_action('admin_bar_menu', 'ocasio_abh_minimalist_admin_bar', 999);

/**
 * 4. Hide "Show Toolbar" setting on profile page for non-administrators
 */
function ocasio_abh_hide_profile_setting() {
	$enabled = get_option('ocasio_abh_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_abh_enabled', 1);
	}
...
add_action('admin_head-profile.php', 'ocasio_abh_hide_profile_setting');
add_action('admin_head-user-edit.php', 'ocasio_abh_hide_profile_setting');

/**
 * 5. Settings Registration
 */
function ocasio_abh_register_settings() {
	register_setting('ocasio_abh_settings_group', 'ocasio_abh_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_abh_register_settings');

/**
 * 6. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_abh_register_menu() {
...
add_action('admin_menu', 'ocasio_abh_register_menu');

/**
 * 7. Action Links on Plugins Screen
 */
function ocasio_abh_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-admin-bar-hider') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_abh_action_links');

/**
 * 8. Conditional Admin Asset Enqueuing
 */
function ocasio_abh_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-abh-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_ABH_VERSION);
		wp_enqueue_script('ocasio-abh-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_ABH_VERSION, true);
		wp_localize_script('ocasio-abh-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-abh-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_abh_admin_assets');
